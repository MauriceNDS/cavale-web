 ...
 * the exercise sheet the athlete just opened. The runner's own copy goes
 * flush to the bottom — that screen hides the tab bar entirely.
 */
export function RestBar({
  ...
  return (
    <div
      className={`fixed inset-x-0 ${layer.restBar} border-y border-moss-200 bg-moss-50/97 px-4 py-2.5 backdrop-blur md:bottom-0 md:border-b-0 md:pb-[max(0.625rem,env(safe-area-inset-bottom))] dark:border-moss-750 dark:bg-moss-900/97 ${
        owned
          ? 'bottom-0 border-b-0 pb-[max(0.625rem,env(safe-area-inset-bottom))]'
