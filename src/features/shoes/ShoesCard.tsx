...
/** The shoe rotation — mileage accrues from validated runs, with a retirement warning. */
          ...
          >
            {editing === shoe.id ? (
              ...
              <ShoeRow
                ...
                onToggleRetired={() =>
                  updateMut.mutate({ id: shoe.id, body: { ...toPayload(shoe), retired: !shoe.retired } })
                }
              />
            )}
          </li>
...
function ShoeForm({
  ...
  function handleSubmit(event: React.FormEvent<HTMLFormElement>) {
    ...
    onSubmit({
      name,
      brand: (data.get('brand') as string).trim() || null,
  ...
  return (
    <form onSubmit={handleSubmit} className="grid gap-3 sm:grid-cols-2">
      ...
      <label className="block text-xs text-moss-500 dark:text-moss-400">
        ...
        <input name="brand" defaultValue={shoe?.brand ?? ''} maxLength={60} className={fieldClass} />
      </label>
      <label className="block text-xs text-moss-500 dark:text-moss-400">
